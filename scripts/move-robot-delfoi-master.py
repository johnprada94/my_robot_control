...
class TrajectoryClient:
    ...
    def __init__(self):
        ...
        # Subscriber to listen for joint positions from the /tcp_server_data topic
        self.joint_positions = None  # Initially set to None until data is received
        rospy.Subscriber("/tcp_server_data", String, self.joint_positions_callback)

    def joint_positions_callback(self, msg):
        """Callback function to update joint positions from the subscriber"""
        try:
            # Split the string, convert to float, and change degrees to radians
            joint_positions = [math.radians(float(pos)) for pos in msg.data.split(",")]
            ...
            rospy.logerr("Failed to convert joint positions from string: {}".format(msg.data))
